...
namespace db
{
    ...
    public class efHelper
    {
        ...
        {
            List<T> list = dc.efConn.Query<T>(cmdText, cmdPara, dc.efTran).AsList<T>();
            return list;
        }
        ...
        {
            T entry = dc.efConn.QueryFirst<T>(cmdText, cmdPara, dc.efTran);
            return entry;
        }
        ...
        public static void checkNotNull(object value, string fieldName)
        {
            if (rui.typeHelper.isNullOrEmpty(value))
                rui.excptHelper.throwEx(string.Format("[{0}]必填", fieldName));
        }
        ...
        /// <param name="value"></param>
        /// <param name="fieldName"></param>
        public static void checkEqualValue(object cValue, string value, string fieldName)
        {
            if (rui.typeHelper.toString(cValue) != value)
                rui.excptHelper.throwEx(string.Format("[{0}]的值要求等于[{1}]", fieldName, value));
        ...
        /// <param name="value"></param>
        /// <param name="fieldName"></param>
        public static void checkNoEqualValue(object cValue, string value, string fieldName)
        {
            if (rui.typeHelper.toString(cValue) == value)
                rui.excptHelper.throwEx(string.Format("[{0}]的值不允许等于[{1}]", fieldName, value));
        }
        ...
        /// <param name="value"></param>
        /// <param name="fieldName"></param>
        public static void checkInt(object value, string fieldName)
        {
            if (rui.typeHelper.toInt(value) == 0)
                rui.excptHelper.throwEx(string.Format("[{0}]的值必须是整数", fieldName));
        }
        ...
        /// <param name="value"></param>
        /// <param name="fieldName"></param>
        public static void checkDecimal(object value, string fieldName)
        {
            if (rui.typeHelper.toDecimal(value) == 0)
                rui.excptHelper.throwEx(string.Format("[{0}]的值必须是数值", fieldName));
        }
        ...
        public static void entryUpdate(object entry, db.dbEntities dc, string exceptField = null, string containField = null)
        {
            ...
            //附加进来
            //.net写法
            dc.Entry(entry).State = System.Data.Entity.EntityState.Unchanged; //没有改变,在保存时不会生成更新语句
            //.netCore写法
