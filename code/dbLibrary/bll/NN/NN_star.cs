...
namespace db.bll
{
    public  class NN_star
    {
        ...
        {
            var model = dc.NN_star.Where(a => a.userID == userID && a.newsID == newsID).SingleOrDefault();
            try
            {
                dc.NN_star.Remove(model);
                dc.SaveChanges();
            }
            catch (Exception ex)
            {
        ...
        public static List<db.NN_newsItem> getNewsModelList(string userID, dbEntities dc)
        {
            ...
            var tmp = dc.NN_star.Where(a => a.userID == userID).ToList<db.NN_star>();
